...
\newline
\newline
where x.y.z. determines the version. For installing on other operating systems, look at R documentations.
...
This value is too small for the seperation factor and a population is combined by mistake. Therefore, we increase septation factor to separate the components more:
